...
st.sidebar.title("GEOSPATIAL RISK PREDICTION \
                 FOR DM & TB")
menu = ["Dashboard Prevalensi DM/TB","Geospatial Factors & Risk Mapping","Analisis Mandiri",]
...
if choice == "Dashboard Prevalensi DM/TB":
    # components.html('''
    ...
    if genre2 == 'Prediksi Risiko Individu':
        ...
        with k1:
            if datapeserta['JK'].values[0] ==0:
                st.image(Image.open('cew.jpeg'))
                # st.image(Image.open('accnt.png'))
            else:
                st.image(Image.open('cow.jpeg'))
